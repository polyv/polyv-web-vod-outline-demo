<template>
  <div class="c-outline-panel" :style="{ backgroundColor: panelBg, boxShadow: panelShadow }">
    ...
    <div class="c-outline-panel__tab-container">
      <div class="c-outline-panel__tab-item" :class="{
        'c-outline-panel__tab-item--active': activeTab === 'outline',
      }" @click="handleTabClick('outline')" :style="{ backgroundColor: tabBg }">
        <span :style="{
          color: activeTab === 'outline' ? whiteColor : grayTextColor,
          borderBottom:
            activeTab === 'outline' ? `3px solid ${whiteColor}` : 'none',
        }">智能大纲</span>
      </div>
      <div class="c-outline-panel__tab-item" :class="{ 'c-outline-panel__tab-item--active': activeTab === 'qa' }"
        ...
        <span :style="{
          color: activeTab === 'qa' ? whiteColor : grayTextColor,
          borderBottom:
            activeTab === 'qa' ? `3px solid ${whiteColor}` : 'none',
        }">AI问答</span>
      </div>
    </div>
    ...
    <div class="c-outline-panel__content">
      ...
      <div v-show="activeTab === 'outline'" class="c-outline-panel__outline-content">
        <!-- 整体loading状态 -->
        <div v-if="isSummaryLoading && !introduction && (!outlineData || outlineData.length === 0)"
             class="c-outline-panel__loading-container">
          <LoadingIndicator :visible="true" :message="summaryLoadingMessage" />
        </div>
        ...
        <template v-else>
          ...
          <div class="c-outline-panel__section-header">
            <h4 class="c-outline-panel__section-title" :style="{ color: whiteColor }">
              摘要
            </h4>
          ...
          <!-- 摘要内容 -->
          <div v-if="introduction" class="c-outline-panel__summary-section"
          ...
          <!-- 摘要加载中 -->
          <div v-else-if="isSummaryLoading" class="c-outline-panel__section-loading">
            <LoadingIndicator :visible="true" message="摘要生成中..." />
          </div>

          <!-- 分段总结标题 -->
          <h4 class="c-outline-panel__section-header" :style="{ color: whiteColor }">
            分段总结
          </h4>
          ...
          <div class="c-outline-panel__segments-section" :style="{ backgroundColor: summaryBg }">
            <!-- 分段数据加载中 -->
            <div v-if="isSummaryLoading && (!outlineData || outlineData.length === 0)"
                 class="c-outline-panel__section-loading">
              <LoadingIndicator :visible="true" message="分段总结生成中..." />
            </div>

            <!-- 无分段数据 -->
            <div v-else-if="!outlineData || outlineData.length === 0" class="c-outline-panel__no-data">
              ...
              <div v-for="(segment, index) in outlineData" :key="segment.id" class="c-outline-panel__segment-item">
                <div class="c-outline-panel__segment-header">
                  <h5 class="c-outline-panel__segment-title" :style="{ color: whiteColor }">
                    {{ segment.title }}
                    <span class="c-outline-panel__segment-time" :style="{ color: brandBlue }"
      ...
      <div v-show="activeTab === 'qa'" class="c-outline-panel__qa-content">
        <LoadingIndicator :visible="isQuestionsLoading &&
          (!suggestQuestions || suggestQuestions.length === 0)
          " :message="questionsLoadingMessage" />
        <!-- 如果没有推荐问题，显示提示 -->
        <div v-if="
          !isQuestionsLoading &&
          (!suggestQuestions || suggestQuestions.length === 0)
        " class="c-outline-panel__no-data">
          <p :style="{ color: grayTextColor }">暂无推荐问题</p>
        </div>

        <!-- 显示推荐问题列表 -->
        <div v-if="suggestQuestions && suggestQuestions.length > 0" class="c-outline-panel__questions-list">
            ...
            <div class="c-outline-panel__question-content">
              <div class="c-outline-panel__question-header">
                <div class="c-outline-panel__question-icon-wrapper">
                  <div class="c-outline-panel__question-icon"></div>
                </div>
              ...
              <div class="c-outline-panel__answer-content">
                <div class="c-outline-panel__answer-icon-wrapper">
                  <div class="c-outline-panel__answer-icon"></div>
                </div>
...
/* Loading状态样式 */
.c-outline-panel__loading-container {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
...
.c-outline-panel__section-loading {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
